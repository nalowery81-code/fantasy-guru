...
(function(){
  ...
  function pctValue(p){const v=p?.rostered_pct??p?.rostered_percentage??p?.percent_rostered??p?.ownership;return F(v)?Number(v):null}
  function pct(p){const v=pctValue(p);return F(v)?`${N(v,0)}%`:'—'}
  ...
  function confidenceRank(p){return({HIGH:4,GOOD:3,MEDIUM:2,LOW:1,UNAVAILABLE:0})[String(p?.weekly_confidence||p?.player_advice?.confidence||'').toUpperCase()]??2}
  ...
  function tvClass(v){if(!F(v))return'';return v>=90?'elite':v>=80?'strong':v>=70?'quality':v>=60?'useful':v>=40?'depth':'replacement'}
  ...
  function sortMetric(p,key){
    if(key==='WEEK')return F(p?.weekly_points)?Number(p.weekly_points):-Infinity;
    if(key==='ROS')return F(p?.ros_points)?Number(p.ros_points):-Infinity;
    if(key==='ROSTERED')return F(pctValue(p))?Number(pctValue(p)):-Infinity;
    if(key==='CONFIDENCE')return confidenceRank(p);
    return F(tv(p))?Number(tv(p)):-Infinity;
  }
  function applyPlayerHubSort(){
    const body=document.querySelector('#ph3Body');if(!body)return;
    const select=document.querySelector('#ph3Sort'),key=select?.value||'TV',rows=[...body.querySelectorAll('tr')];
    const ranked=rows.map((row,index)=>{const btn=row.querySelector('[data-ph3-id]'),id=btn?.dataset.ph3Id||'',name=row.querySelector('.ph3Player b')?.textContent||'',p=findPlayer(name,id);return{row,p,index}}).sort((a,b)=>{const d=sortMetric(b.p,key)-sortMetric(a.p,key);return Number.isFinite(d)&&d!==0?d:a.index-b.index});
    ranked.forEach((x,i)=>{const rank=x.row.cells?.[0];if(rank)rank.textContent=String(i+1);body.appendChild(x.row)})
  }
  function ensurePlayerHubSort(){
    const page=document.querySelector('.ph3Page'),controls=page?.querySelector('.ph3Search');if(!page||!controls)return;
    if(!document.querySelector('#ph3Sort')){
      const s=document.createElement('select');s.id='ph3Sort';s.setAttribute('aria-label','Sort players');s.innerHTML='<option value="TV">Sort: Trade Value</option><option value="WEEK">Sort: Week</option><option value="ROS">Sort: ROS</option><option value="ROSTERED">Sort: Rostered</option><option value="CONFIDENCE">Sort: Confidence</option>';s.onchange=applyPlayerHubSort;controls.appendChild(s)
    }
    applyPlayerHubSort();
  }
  ...
  document.addEventListener('click',e=>{
    ...
    if(ph){e.preventDefault();const p=findPlayer(playerNameFromButton(ph),ph.dataset.ph3Id||'');if(p)showPlayerHubDetail(p);return}
  });

  const observer=new MutationObserver(()=>{startSitDetail(false);ensurePlayerHubSort()});
  observer.observe(document.body,{childList:true,subtree:true});
  setTimeout(()=>{startSitDetail(false);ensurePlayerHubSort()},0);

...
(function(){
  ...
  function findPlayer(name){const q=String(name||'').trim().toLowerCase();return allPlayers().find(p=>String(p?.name||'').trim().toLowerCase()===q)||null}
  ...
  function showPlayerHubDetail(p){
    const page=document.querySelector('.ph2Page'),cards=page?.querySelector('.ph2Cards');if(!page||!cards||!p)return;
    let el=page.querySelector('#ph2Detail');if(!el){el=document.createElement('section');el.id='ph2Detail';el.className='fgIntDetail';cards.after(el)}
    el.innerHTML=`<div class="fgIntHead"><div class="fgIntTitle">${avatar(p)}<div><h3>${E(p.name||'Player')}</h3><p>${E(POS(p))} · ${E(p.team||'Free Agent')} · Opp ${E(opponent(p))}</p></div></div><button id="ph2DetailClose" aria-label="Close player details">×</button></div><div class="fgIntStats"><div><span>Week</span><b>${N(p.weekly_points)}</b></div><div><span>ROS</span><b>${N(p.ros_points)}</b></div><div><span>Rostered</span><b>${pct(p)}</b></div><div><span>ESPN</span><b>${N(p.espn_weekly_points)}</b></div><div><span>Sleeper</span><b>${N(p.sleeper_weekly_points)}</b></div><div><span>ffanalytics</span><b>${N(p.ffanalytics_weekly_points)}</b></div><div><span>Confidence</span><b>${E(confidence(p))}</b></div></div>`;
    const close=el.querySelector('#ph2DetailClose');if(close)close.onclick=()=>el.remove();
    el.querySelectorAll('img').forEach(x=>x.onerror=()=>x.style.display='none');
    requestAnimationFrame(()=>el.scrollIntoView({behavior:'smooth',block:'nearest'}));
  }

  function playerNameFromButton(btn){
    const row=btn.closest('tr'),card=btn.closest('.ph2Card');
    return row?.querySelector('.ph2Player b')?.textContent||card?.querySelector('.ph2HeroPlayer b')?.textContent||'';
  }
  ...
  document.addEventListener('click',e=>{
    ...
    const ph=e.target.closest('.ph2View[data-ph-waiver="0"],.ph2Btn[data-ph-action="view"]');
    if(ph){
      e.preventDefault();
      const p=findPlayer(playerNameFromButton(ph));

...
export function attachTradeValues(context,data){
  ...
  const corrVals=[corrRosVorp,corrRosStarter,corrVorpStarter].filter(finite).map(x=>Math.abs(Number(x))),avgCorr=corrVals.length?corrVals.reduce((s,v)=>s+v,0)/corrVals.length:null;
  const auditStatus=scored<12?'INSUFFICIENT_SAMPLE':finite(avgCorr)&&avgCorr>=.80?'CAUTION':'PASS';
  data.trade_value_audit={version:'tv-v3-audit',platform:String(context?.platform||'UNKNOWN').toUpperCase(),players_evaluated:scored,projection_linked_weight_pct:75,correlations:{ros_vs_vorp:finite(corrRosVorp)?Math.round(corrRosVorp*100)/100:null,ros_vs_starter_impact:finite(corrRosStarter)?Math.round(corrRosStarter*100)/100:null,vorp_vs_starter_impact:finite(corrVorpStarter)?Math.round(corrVorpStarter*100)/100:null,average_absolute:finite(avgCorr)?Math.round(avgCorr*100)/100:null},warning_threshold:.80,status:auditStatus,missing_component_fallbacks:{market:marketFallbacks,week_outlook:weekFallbacks},rule:'Audit only. A CAUTION status means the three projection-linked TV components are highly correlated across this league; it does not change player scores automatically.'};

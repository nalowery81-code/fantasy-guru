...
const playerKey=p=>String(p?.canonical_player_id||p?.sleeper_id||p?.espn_id||p?.id||`${p?.name}|${pos(p?.position)}`);
...
export function attachTradeValues(context,data){
  ...
  for(const p of all){const k=playerKey(p);if(!k||seen.has(k))continue;seen.add(k);unique.push(p)}
  const waiverKeys=new Set(waivers.map(playerKey));
  const shape=leagueShape(context),byPos={},waiverByPos={};
  for(const p of unique){const q=pos(p.position);(byPos[q]||(byPos[q]=[])).push(p)}
  for(const p of waivers){const q=pos(p.position);if(finite(p.ros_points))(waiverByPos[q]||(waiverByPos[q]=[])).push(p)}
  ...
  for(const [q,group] of Object.entries(byPos)){
    ...
    const waiverVals=(waiverByPos[q]||[]).filter(p=>finite(p.ros_points)).map(p=>Number(p.ros_points)).sort((a,b)=>b-a),waiverRepl=averageTop(waiverVals,3);
  ...
  for(const [q,group] of Object.entries(byPos)){
    const rosVals=group.map(p=>p.ros_points).filter(finite),weekVals=group.map(p=>p.weekly_points).filter(finite),topRos=rosVals.length?Math.max(...rosVals.map(Number)):null,cut=cutoffs[q]||1;
    ...
    for(const p of group){
      if(!finite(p.ros_points)){Object.assign(p,{trade_value:null,tv:null,trade_value_label:'Unavailable',trade_value_version:'tv-v4-ros'});continue}
      let repl=replacement[q],replSource=replacementSource[q],replSamples=replacementSamples[q];
      if(waiverKeys.has(playerKey(p))){
        const alternatives=(waiverByPos[q]||[]).filter(w=>playerKey(w)!==playerKey(p)&&finite(w.ros_points)).map(w=>Number(w.ros_points));
        const altRepl=averageTop(alternatives,3);
        if(finite(altRepl)){
          repl=Number(altRepl);replSource='TOP_3_AVAILABLE_EXCLUDING_SELF_AVG';replSamples=Math.min(3,alternatives.length);
        }
      }
      ...
      scored++;
      Object.assign(p,{trade_value:tv,tv,trade_value_label:label(tv),trade_value_version:'tv-v4-ros',trade_value_horizon:'REST_OF_SEASON',trade_value_components:{ros_projection:Math.round(rosScore),value_over_replacement:Math.round(vorp),market:Math.round(marketScore),market_observed:finite(observedMarket)?Math.round(observedMarket):null,market_fallback_used:!finite(observedMarket),starter_impact:Math.round(starterImpact),week_outlook:Math.round(weekOutlook),weekly_trend:Math.round(weekOutlook),week_outlook_observed:finite(observedWeek)?Math.round(observedWeek):null,week_outlook_fallback_used:!finite(observedWeek),confidence_risk:Math.round(risk),replacement_ros_points:finite(repl)?Math.round(Number(repl)*10)/10:null,replacement_source:replSource,replacement_sample_size:replSamples,position_rank:r,starter_cutoff:cut}});
    }
  }

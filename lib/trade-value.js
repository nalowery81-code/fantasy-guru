...
export function attachTradeValues(context,data){
  ...
  for(const [q,group] of Object.entries(byPos)){
    ...
    for(const p of group){
      if(!finite(p.ros_points)){Object.assign(p,{trade_value:null,tv:null,trade_value_label:'Unavailable',trade_value_version:'tv-v2-ros'});continue}
      ...
      Object.assign(p,{trade_value:tv,tv,trade_value_label:label(tv),trade_value_version:'tv-v2-ros',trade_value_horizon:'REST_OF_SEASON',trade_value_components:{ros_projection:Math.round(rosScore),value_over_replacement:Math.round(vorp),market:finite(marketScore)?Math.round(marketScore):null,starter_impact:Math.round(starterImpact),weekly_trend:finite(weeklyScore)?Math.round(weeklyScore):null,confidence_risk:Math.round(risk),replacement_ros_points:finite(repl)?Math.round(Number(repl)*10)/10:null,replacement_source:replacementSource[q],replacement_sample_size:replacementSamples[q],position_rank:r,starter_cutoff:cut}});
    }
  }
  ...
  data.trade_value_policy={version:'tv-v2-ros',horizon:'REST_OF_SEASON',scale:'0-100',weights:{ros_projection:45,value_over_replacement:20,market_value:15,starter_impact:10,weekly_trend:5,confidence_and_injury_risk:5},market_source:'FantasyCalc',replacement_rule:'VORP compares a player to the average ROS projection of the top three actually available players at the same position. Starter impact remains a separate league-specific component. If no waiver baseline exists, a deeper rosterable cutoff is used as fallback.',rule:'TV is a trade asset score, not a fantasy-point projection. Missing optional components are reweighted across available evidence.',position_caps:{K:45,DEF:50}};
  return data;
}

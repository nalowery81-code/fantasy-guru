...
export function attachTradeValues(context,data){
  const all=[...(data?.team_details||[]).flatMap(t=>t.players||[]),...(data?.waiver_pool||[])];
  ...
  const shape=leagueShape(context),byPos={};for(const p of unique){const q=pos(p.position);(byPos[q]||(byPos[q]=[])).push(p)}
  const marketValues=unique.map(p=>p.market_value).filter(finite),cutoffs={},replacement={};
  for(const [q,group] of Object.entries(byPos)){
    ...
    const cut=starterCutoff(q,shape);cutoffs[q]=cut;replacement[q]=vals.length?Number(vals[Math.min(vals.length-1,Math.max(0,cut-1))].ros_points):null;
  }
  for(const [q,group] of Object.entries(byPos)){
    ...
    for(const p of group){
      if(!finite(p.ros_points)){Object.assign(p,{trade_value:null,tv:null,trade_value_label:'Unavailable',trade_value_version:'tv-v1-ros'});continue}
      ...
      const marketScore=finite(p.market_value)?percentile(p.market_value,marketValues):null;
      const r=rankMap.get(p)||999,starterImpact=clamp(100*(1-(r-1)/(Math.max(1,cut)*1.5)));
      const weeklyScore=finite(p.weekly_points)?percentile(p.weekly_points,weekVals):null;
      const risk=confidenceScore(p);
      const components=[['ros',rosScore,45],['vorp',vorp,20],['market',marketScore,15],['starter',starterImpact,10],['weekly',weeklyScore,5],['risk',risk,5]].filter(([,v])=>finite(v));
      const weight=components.reduce((n,x)=>n+x[2],0)||1,tvRaw=components.reduce((n,x)=>n+Number(x[1])*x[2],0)/weight;
      let tv=Math.round(clamp(tvRaw));
      if(q==='K')tv=Math.min(tv,45);if(q==='DEF')tv=Math.min(tv,50);
      Object.assign(p,{trade_value:tv,tv,trade_value_label:label(tv),trade_value_version:'tv-v1-ros',trade_value_horizon:'REST_OF_SEASON',trade_value_components:{ros_projection:Math.round(rosScore),value_over_replacement:Math.round(vorp),market:finite(marketScore)?Math.round(marketScore):null,starter_impact:Math.round(starterImpact),weekly_trend:finite(weeklyScore)?Math.round(weeklyScore):null,confidence_risk:Math.round(risk),replacement_ros_points:finite(repl)?Math.round(Number(repl)*10)/10:null,position_rank:r,starter_cutoff:cut}});
    }
  }
  data.trade_value_policy={version:'tv-v1-ros',horizon:'REST_OF_SEASON',scale:'0-100',weights:{ros_projection:45,value_over_replacement:20,market_value:15,starter_impact:10,weekly_trend:5,confidence_and_injury_risk:5},market_source:'FantasyCalc',rule:'TV is a trade asset score, not a fantasy-point projection. Missing optional components are reweighted across available evidence.',position_caps:{K:45,DEF:50}};
  return data;
}

...
function findStat(p,{season,week,source,split}){
  ...
  if(Number(split)===1){
    // ESPN weekly player-pool rows may omit seasonId. Match the same way the
    // verified league parser does: source + weekly split + scoring period.
    return stats.find(s=>Number(s?.statSourceId)===Number(source)&&Number(s?.statSplitTypeId)===1&&Number(s?.scoringPeriodId)===Number(week)&&s?.stats&&typeof s.stats==='object')||null;
  }
  return stats.find(s=>Number(s?.seasonId)===Number(season)&&Number(s?.statSourceId)===Number(source)&&Number(s?.statSplitTypeId)===Number(split)&&s?.stats&&typeof s.stats==='object')||null;
}

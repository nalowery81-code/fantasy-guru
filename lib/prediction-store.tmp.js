...
export async function listPredictionRecords({ season, week, platform, leagueId, status, limit = 100 } = {}) {
  ...
  if (platform) q.set('platform', `eq.${String(platform).toLowerCase()}`);
  if (leagueId) q.set('league_id', `eq.${String(leagueId)}`);

...
const ffaWeeklyData=JSON.parse(readFileSync(join(process.cwd(),'data','ffanalytics_weekly.json'),'utf8'));
...
async function nflGames(context){
  ...
  try{
    ...
    for(const e of d?.events||[]){
      ...
      const indoor=c?.venue?.indoor===true||/dome|indoor/i.test(String(c?.venue?.fullName||''));
      const common={event_id:e?.id||null,game_total:total,spread_details:od?.details||null,weather:wt,wind_mph:wind,venue:c?.venue?.fullName||null,indoor};

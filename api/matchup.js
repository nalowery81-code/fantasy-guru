...
function espnRosterActuals(entries=[],week){const out={};for(const e of entries||[]){const p=e?.playerPoolEntry?.player||e?.player||{},pts=espnEntryActual(e,week);if(!finite(pts))continue;const id=String(p.id||e?.playerId||e?.playerPoolEntry?.id||'');if(id)out[id]=Number(pts);const name=p.fullName||p.name||'',pos=({1:'QB',2:'RB',3:'WR',4:'TE',5:'K',16:'DEF'})[p.defaultPositionId]||String(p.defaultPositionId||'');if(name)out[norm(name)+'|'+posNorm(pos)]=Number(pts)}return out}
function espnStarterTotal(entries=[],week){let total=0,found=false;for(const e of entries||[]){const slot=Number(e?.lineupSlotId);if(slot===20||slot===21)continue;const pts=espnEntryActual(e,week);if(!finite(pts))continue;total+=Number(pts);found=true}return found?total:null}
async function espnLiveMatchup(context){
  ...
  const url=`${base}?view=mBoxscore&view=mLiveScoring&view=mScoreboard&view=mStatus&view=mRoster&scoringPeriodId=${week}&matchupPeriodId=${week}`;
  const r=await fetch(url,{headers});if(!r.ok)return null;const data=await r.json();let opponentId=null,myTotal=null,oppTotal=null,myActuals={},oppActuals={};
  const schedule=(data?.schedule||[]).filter(m=>{const home=Number(m?.home?.teamId),away=Number(m?.away?.teamId);return home===myId||away===myId});
  const exactMatchup=schedule.find(m=>Number(m?.matchupPeriodId)===week),exactScoring=schedule.find(m=>Number(m?.scoringPeriodId)===week),selected=exactMatchup||exactScoring||(schedule.length===1?schedule[0]:null);
  if(selected){const m=selected,home=Number(m?.home?.teamId),away=Number(m?.away?.teamId),mySide=home===myId?m.home:m.away,oppSide=home===myId?m.away:m.home;if(oppSide?.teamId){opponentId=Number(oppSide.teamId);const pickEntries=s=>s?.rosterForMatchupPeriod?.entries||s?.rosterForCurrentScoringPeriod?.entries||s?.roster?.entries||[];const myEntries=pickEntries(mySide),oppEntries=pickEntries(oppSide);myActuals=espnRosterActuals(myEntries,week);oppActuals=espnRosterActuals(oppEntries,week);const myStarterTotal=espnStarterTotal(myEntries,week),oppStarterTotal=espnStarterTotal(oppEntries,week);myTotal=finite(myStarterTotal)?Number(myStarterTotal):null;oppTotal=finite(oppStarterTotal)?Number(oppStarterTotal):null}}
  if(!opponentId)return null;
  if(!Object.keys(myActuals).length||!Object.keys(oppActuals).length){const fallback=await fetch(`${base}?view=mRoster&scoringPeriodId=${week}`,{headers}).then(x=>x.ok?x.json():null).catch(()=>null);const mine=(fallback?.teams||[]).find(t=>Number(t.id)===myId),opp=(fallback?.teams||[]).find(t=>Number(t.id)===Number(opponentId));if(!Object.keys(myActuals).length)myActuals=espnRosterActuals(mine?.roster?.entries||[],week);if(!Object.keys(oppActuals).length)oppActuals=espnRosterActuals(opp?.roster?.entries||[],week);if(!finite(myTotal))myTotal=espnStarterTotal(mine?.roster?.entries||[],week);if(!finite(oppTotal))oppTotal=espnStarterTotal(opp?.roster?.entries||[],week)}
  return{opponentId:Number(opponentId),myActuals,oppActuals,myTotal,oppTotal,refreshedAt:new Date().toISOString(),scoreSource:`ESPN Week ${week} live boxscore`}
}

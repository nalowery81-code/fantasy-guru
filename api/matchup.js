...
async function espnLiveMatchup(context){
  ...
  const url=`${base}?view=mBoxscore&view=mLiveScoring&view=mScoreboard&view=mStatus&scoringPeriodId=${week}&matchupPeriodId=${week}`;
  ...
  for(const m of data?.schedule||[]){const home=Number(m?.home?.teamId),away=Number(m?.away?.teamId);if(home!==myId&&away!==myId)continue;const mySide=home===myId?m.home:m.away,oppSide=home===myId?m.away:m.home;if(!oppSide?.teamId)continue;opponentId=Number(oppSide.teamId);const myEntries=mySide?.rosterForCurrentScoringPeriod?.entries||[],oppEntries=oppSide?.rosterForCurrentScoringPeriod?.entries||[];myActuals=espnRosterActuals(myEntries,week);oppActuals=espnRosterActuals(oppEntries,week);const myStarterTotal=espnStarterTotal(myEntries,week),oppStarterTotal=espnStarterTotal(oppEntries,week);myTotal=finite(myStarterTotal)?Number(myStarterTotal):(finite(mySide?.totalPoints)?Number(mySide.totalPoints):null);oppTotal=finite(oppStarterTotal)?Number(oppStarterTotal):(finite(oppSide?.totalPoints)?Number(oppSide.totalPoints):null);break}
  ...
  return{opponentId:Number(opponentId),myActuals,oppActuals,myTotal,oppTotal,refreshedAt:new Date().toISOString(),scoreSource:'ESPN live boxscore'}
}

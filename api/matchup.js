...
async function espnLiveMatchup(context){
  ...
  for(const m of data?.schedule||[]){const home=Number(m?.home?.teamId),away=Number(m?.away?.teamId);if(home!==myId&&away!==myId)continue;const mySide=home===myId?m.home:m.away,oppSide=home===myId?m.away:m.home;if(!oppSide?.teamId)continue;opponentId=Number(oppSide.teamId);const pickEntries=s=>s?.rosterForMatchupPeriod?.entries||s?.rosterForCurrentScoringPeriod?.entries||s?.roster?.entries||[];const myEntries=pickEntries(mySide),oppEntries=pickEntries(oppSide);myActuals=espnRosterActuals(myEntries,week);oppActuals=espnRosterActuals(oppEntries,week);const myStarterTotal=espnStarterTotal(myEntries,week),oppStarterTotal=espnStarterTotal(oppEntries,week);myTotal=finite(myStarterTotal)?Number(myStarterTotal):null;oppTotal=finite(oppStarterTotal)?Number(oppStarterTotal):null;break}

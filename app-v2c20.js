...
(function(){
  ...
  function pickerHTML(side,select){
  ...
  function renderList(box,select){
    ...
    let rows=playerPoolFor(side);if(pf!=='ALL')rows=rows.filter(p=>pos(p.position)===pf);if(q)rows=rows.filter(p=>String(p.name||'').toLowerCase().includes(q));rows=sortRows(rows,sm).slice(0,120);
  ...
  function enhanceCompare(){
    const drop=$('fg17Drop'),add=$('fg17Add');if(!drop||!add||document.querySelector('.fg20CompareEnhance'))return;
    const host=document.querySelector('.fg17ComparePick');if(!host)return;
    ...
    host.insertAdjacentHTML('afterend',`<div class="fg20Pickers">${pickerHTML('drop',drop)}${pickerHTML('add',add)}</div>`);
    document.querySelectorAll('.fg20Picker').forEach(box=>{const sel=box.dataset.side==='drop'?drop:add;['input','change'].forEach(ev=>box.querySelector('.fg20Search').addEventListener(ev,()=>renderList(box,sel)));box.querySelector('.fg20Pos').onchange=()=>renderList(box,sel);box.querySelector('.fg20Sort').onchange=()=>renderList(box,sel);renderList(box,sel)});
  ...
  const obs=new MutationObserver(()=>{if(document.getElementById('fg17Drop')&&document.getElementById('fg17Add'))requestAnimationFrame(enhanceCompare)});obs.observe(document.documentElement,{subtree:true,childList:true});setTimeout(enhanceCompare,0);
})();

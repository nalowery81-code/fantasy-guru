...
(function(){
  ...
  function heldDecisionCards(opt){
    const held=(opt||[]).filter(x=>x?.held_for_uncertainty&&x?.player&&x?.raw_player);
    if(!held.length)return'';
    return `<section class="fg33Held"><div class="fg33HeldHead"><div><h3>Close Calls Guru Held</h3><p>Guru saw the higher raw projection, but the edge was not strong enough to justify changing your starter.</p></div><span>${held.length}</span></div>${held.map(x=>{
      const current=x.player,challenger=x.raw_player,edge=Number(x.held_edge||0),required=Number(x.required_edge||0);
      return `<article class="fg33HeldCard"><div class="fg33HeldMain"><span class="fg33Action hold">HOLD</span><div><b>${E(current.name)}</b><small>over ${E(challenger.name)}</small></div><div class="fg33HeldEdge"><b>${edge>=0?'+':''}${N(edge)}</b><small>raw challenger edge</small></div></div><p>${E(challenger.name)} projects slightly higher, but the ${edge>=0?'+':''}${N(edge)}-point edge does not clear Guru's ${N(required)}-point uncertainty threshold.</p><details class="fg33Analytics"><summary>See Why</summary><div class="fg33DetailGrid"><span>${E(current.name)} week <b>${N(current.weekly_points)}</b></span><span>${E(challenger.name)} week <b>${N(challenger.weekly_points)}</b></span><span>Raw edge <b>${edge>=0?'+':''}${N(edge)}</b></span><span>Required edge <b>${N(required)}</b></span><span>Decision <b>Keep ${E(current.name)}</b></span><span>Reason <b>Projection edge is inside expected uncertainty.</b></span></div></details></article>`;
    }).join('')}</section>`;
  }
  ...
  const style=document.createElement('style');style.textContent=`
    ...
    .fg33Held{margin:0 0 12px;border:1px solid #2d5e86;border-radius:11px;background:#091727;padding:10px}.fg33HeldHead{display:flex;justify-content:space-between;gap:12px;align-items:center;padding:2px 2px 8px}.fg33HeldHead h3{margin:0;font-size:13px}.fg33HeldHead p{margin:2px 0 0;color:#8fa6bf;font-size:9px}.fg33HeldHead>span{min-width:24px;height:24px;border-radius:999px;display:grid;place-items:center;background:#10243a;border:1px solid #2d5e86;color:#b9dcf5;font-size:9px;font-weight:900}.fg33HeldCard{border-top:1px solid #1e324a;padding:10px 2px 2px}.fg33HeldMain{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:9px;align-items:center}.fg33HeldMain>div>b{display:block;font-size:12px}.fg33HeldMain>div>small,.fg33HeldEdge small{display:block;color:#8fa6bf;font-size:8px;margin-top:2px}.fg33HeldEdge{text-align:right}.fg33HeldEdge>b{font-size:12px;color:#fde68a}.fg33HeldCard>p{margin:8px 0 0;color:#c7d5e5;font-size:9px;line-height:1.45}
    ...
    @media(max-width:600px){.fg33DetailGrid{grid-template-columns:1fr}.fg33RosterMain{grid-template-columns:58px minmax(0,1fr) 54px}.fg33HeldMain{grid-template-columns:auto 1fr}.fg33HeldEdge{grid-column:2;text-align:left}}

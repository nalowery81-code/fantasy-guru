...
(function(){
  ...
  function tvAuditHTML(){
    const a=analysis?.trade_value_audit||null;
    ...
    const c=a.correlations||{},f=a.missing_component_fallbacks||{},status=String(a.status||'UNKNOWN'),statusText=status==='PASS'?'PASS — projection-linked components are not excessively correlated in this league.':status==='CAUTION'?'CAUTION — ROS, VORP and Starter Impact are highly correlated in this league. Review before changing weights.':'INSUFFICIENT SAMPLE — keep collecting player observations before judging component overlap.';
    return`<div class="card" style="margin-top:14px"><h3>Trade Value Model Audit</h3><div><b>${esc(status)}</b> · ${esc(a.platform||C?.platform||'UNKNOWN')} · TV ${esc(analysis?.trade_value_policy?.version||'')}</div><div class="muted" style="margin-top:5px">${esc(statusText)}</div><div class="dashboardGrid" style="margin-top:10px"><div class="card"><div class="muted">Projection-linked weight</div><h3 style="font-size:24px;margin:5px 0">${Number(a.projection_linked_weight_pct||0)}%</h3><div class="muted">ROS + VORP + Starter Impact</div></div><div class="card"><div class="muted">Average absolute correlation</div><h3 style="font-size:24px;margin:5px 0">${fmt(c.average_absolute)}</h3><div class="muted">Caution threshold ${fmt(a.warning_threshold)}</div></div><div class="card"><div class="muted">Missing-evidence fallbacks</div><h3 style="font-size:24px;margin:5px 0">${Number(f.market||0)+Number(f.week_outlook||0)}</h3><div class="muted">Market ${Number(f.market||0)} · Week Outlook ${Number(f.week_outlook||0)}</div></div></div><div class="row"><b>ROS ↔ VORP</b><div class="muted">Correlation ${fmt(c.ros_vs_vorp)}</div></div><div class="row"><b>ROS ↔ Starter Impact</b><div class="muted">Correlation ${fmt(c.ros_vs_starter_impact)}</div></div><div class="row"><b>VORP ↔ Starter Impact</b><div class="muted">Correlation ${fmt(c.vorp_vs_starter_impact)}</div></div><div class="muted" style="margin-top:7px">This audit is diagnostic only. It never changes a player's TV automatically.</div></div>`;
  }

...
(function(){
  ...
  function updateTray(){
    const tray=document.querySelector('.fg21Tray'),drop=$('fg17Drop'),add=$('fg17Add');if(!tray||!drop||!add)return false;
    const next=`${selectedCard('drop',drop.value)}<div class="fg21Vs">⇄</div>${selectedCard('add',add.value)}`;
    if(tray.innerHTML!==next)tray.innerHTML=next;
    return true;
  }
  function enhance(){
    const drop=$('fg17Drop'),add=$('fg17Add'),pickers=document.querySelector('.fg20Pickers');if(!drop||!add||!pickers)return false;
  ...
  function scheduleEnhance(attempt=0){requestAnimationFrame(()=>{if(enhance())return;if(attempt<5)setTimeout(()=>scheduleEnhance(attempt+1),30)})}
  ...
  document.addEventListener('click',e=>{const tab=e.target.closest?.('.fg17Tab');if(tab?.dataset.tab==='compare')setTimeout(()=>scheduleEnhance(),20);if(e.target.closest?.('.fg20Row'))setTimeout(updateTray,0)});
  const baseShow=showView;showView=function(v){const out=baseShow(v);if(v==='playerhub')setTimeout(()=>scheduleEnhance(),20);return out};

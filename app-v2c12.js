...
function fg12ProjectionDetail(){
  ...
  box=document.createElement('div');box.className='fg12InsightDetail';box.innerHTML=`<div><b>Live projected finish</b><span>${esc(d.you.team)} ${Number(d.you.projection||0).toFixed(1)} · ${esc(d.them.team)} ${Number(d.them.projection||0).toFixed(1)}</span></div><div><b>Live projected edge</b><span>${edge>=0?esc(d.you.team):esc(d.them.team)} by ${Math.abs(edge).toFixed(1)} points</span></div><div><b>Live scoring edge</b><span>${live===0?'Tied right now':`${live>0?esc(d.you.team):esc(d.them.team)} by ${Math.abs(live).toFixed(1)} points`}</span></div>`;alerts.insertAdjacentElement('afterend',box)
}
